...
image = cv2.imread('trainning_images/test3.jpg')
img = preprocessimg(getCards(image))[0:120, 0:55]
print matchCards(img,trainCards())
cv2.imshow('The card after rotation',img)
cv2.imshow('trainset',trainCards()['Diamond of Three'])
cv2.waitKey(0)
cv2.destroyAllWindows()
'''template = cv2.imread('crop_trainset/trainning_images/Diamond of King.jpg')
template_gray = preprocessimg(template)

result = cv2.matchTemplate(img, template_gray, cv2.CV_TM_SQDIFF)
print result'''